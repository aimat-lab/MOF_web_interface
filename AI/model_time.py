...
def model_time(MOF_random_name = None, df_new = None, startpath = None):
    ...
    csvfilename="%s/datasets/rac_features_time.csv"%(startpath)
    ...
    if not os.path.exists("%s/prediction_data/scatter_plots_time"%(outdir)):
        os.makedirs("%s/prediction_data/scatter_plots_time"%(outdir))

    if not os.path.exists("%s/models/models_time"%(outdir)):
        os.makedirs("%s/models/models_time"%(outdir))

    if not os.path.exists("%s/prediction_data/predictions_rawdata_time"%(outdir)):
        os.makedirs("%s/prediction_data/predictions_rawdata_time"%(outdir))
    ...
    for train_index, test_index in kf.split(X):
        ...
        if not os.path.exists("%s/models/models_time/random_forest_regression_%i.joblib"%(outdir, counter)):
            ...
            joblib.dump(model, "%s/models/models_time/random_forest_regression_%i.joblib"%(outdir, counter))
            joblib.dump(x_scaler_feat, "%s/models/models_time/random_forest_regression_%i_x_scaler.joblib"%(outdir, counter))
            joblib.dump(y_scaler, "%s/models/models_time/random_forest_regression_%i_y_scaler.joblib"%(outdir, counter))
            ...
            np.savetxt("%s/prediction_data/predictions_rawdata_time/y_real_"%(outdir)+str(counter)+"_test.txt", y_test_unscaled)
            np.savetxt("%s/prediction_data/predictions_rawdata_time/y_real_"%(outdir)+str(counter)+"_train.txt", y_train_unscaled)
            np.savetxt("%s/prediction_data/predictions_rawdata_time/y_RFR_"%(outdir)+str(counter)+"_test.txt", y_pred_test_unscaled)
            np.savetxt("%s/prediction_data/predictions_rawdata_time/y_RFR_"%(outdir)+str(counter)+"_train.txt", y_pred_train_unscaled)
            ...
            plt.savefig("%s/prediction_data/scatter_plots_time/full_data_RFR_%i.png"%(outdir, counter),dpi=300)
            plt.close()

        else:
            model = joblib.load("%s/models/models_time/random_forest_regression_%i.joblib"%(outdir, counter))
            x_scaler_feat = joblib.load("%s/models/models_time/random_forest_regression_%i_x_scaler.joblib"%(outdir, counter))
            y_scaler = joblib.load("%s/models/models_time/random_forest_regression_%i_y_scaler.joblib"%(outdir, counter))
            x_new_scaled = x_scaler_feat.transform(x_new_unscaled).reshape(1, -1)
        ...
        single_prediction.write(str(y_new_pred_unscaled[0])+'\n')
        ### Here we scale back the output
