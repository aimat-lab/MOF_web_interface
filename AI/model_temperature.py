...
def reg_stats(y_true,y_pred,scaler=None):
    ...
    csvfilename="rac_features_temperature.csv"
...
def model_temperature(MOF_random_name = None, df_new = None):
    ...
    if not os.path.exists("%s/prediction_data/scatter_plots_temperature"%(outdir)):
        os.makedirs("%s/prediction_data/scatter_plots_temperature"%(outdir))

    if not os.path.exists("%s/models/models_temperature"%(outdir)):
        os.makedirs("%s/models/models_temperature"%(outdir))

    if not os.path.exists("%s/prediction_data/predictions_rawdata_temperature"%(outdir)):
        os.makedirs("%s/prediction_data/predictions_rawdata_temperature"%(outdir))
    ...
    for train_index, test_index in kf.split(X):
        ...
        if not os.path.exists("%s/models/models_temperature/random_forest_regression_%i.joblib"%(outdir, counter)):
            ...
            joblib.dump(model, "%s/models/models_temperature/random_forest_regression_%i.joblib"%(outdir, counter))
            joblib.dump(x_scaler_feat, "%s/models/models_temperature/random_forest_regression_%i_x_scaler.joblib"%(outdir, counter))
            joblib.dump(y_scaler, "%s/models/models_temperature/random_forest_regression_%i_y_scaler.joblib"%(outdir, counter))
            ...
            plt.savefig("%s/prediction_data/scatter_plots_temperature/full_data_RFR_%i.png"%(outdir, counter),dpi=300)
            plt.close()

    
        else:
            model = joblib.load("%s/models/models_temperature/random_forest_regression_%i.joblib"%(outdir, counter))
            x_scaler_feat = joblib.load("%s/models/models_temperature/random_forest_regression_%i_x_scaler.joblib"%(outdir, counter))
            y_scaler = joblib.load("%s/models/models_temperature/random_forest_regression_%i_y_scaler.joblib"%(outdir, counter))
            x_new_scaled = x_scaler_feat.transform(x_new_unscaled).reshape(1, -1)
