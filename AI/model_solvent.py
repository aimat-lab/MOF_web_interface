...
def model_solvent(MOF_random_name = None, df_new = None):
    ...
    csvfilename="datasets/rac_features_solvent.csv"
    ...
    single_prediction=open('predictions/%s_solvent_prediction.dat'%(MOF_random_name),'w')
    ...
    for train_index, test_index in kf.split(X):
        ...
        if not os.path.exists("%s/models/models_solvent/random_forest_regression_%i.joblib"%(outdir, counter)):
           ...
           ############################
            ...
            np.savetxt("./prediction_data/predictions_rawdata_solvent/y_real_"+str(counter)+"_test.txt", y_test_unscaled)
            np.savetxt("./prediction_data/predictions_rawdata_solvent/y_real_"+str(counter)+"_train.txt", y_train_unscaled)
            np.savetxt("./prediction_data/predictions_rawdata_solvent/y_RFR_"+str(counter)+"_test.txt", y_pred_test_unscaled)
            np.savetxt("./prediction_data/predictions_rawdata_solvent/y_RFR_"+str(counter)+"_train.txt", y_pred_train_unscaled)
